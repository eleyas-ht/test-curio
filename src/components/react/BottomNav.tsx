...
export default function BottomNav({ pathname }: Props) {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-line bg-snow/95 backdrop-blur-md md:hidden"
    ...
    >
      ...
      >
        <span className="relative">
          ...
          {count > 0 && (
            <span className="absolute -right-2 -top-1.5 grid h-4 min-w-4 place-items-center rounded-full bg-accent-strong px-1 font-mono text-[9px] font-semibold leading-none text-white">
              {count}
            </span>
          )}
        </span>
