...
import { buyerIpFrom, countryFrom, setCartNote, json } from '~/lib/cart-server';
...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    const { cart, userErrors } = await setCartNote(cookies, note, buyerIpFrom(request), countryFrom(request));
