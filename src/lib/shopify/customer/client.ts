  ...
  return {
    ...
    async query<T>(query: string, variables: Record<string, unknown> = {}): Promise<T> {
      ...
      try {
        ...
        throw new Error(`Customer Account API HTTP ${res.status} ${res.statusText} (non-JSON body)`);
      }
      if (!res.ok) {
        // Include Shopify's error details from the response body when available.
        const detail = (json as Record<string, unknown>).errors
          ? JSON.stringify((json as Record<string, unknown>).errors)
          : (json as Record<string, unknown>).error ?? res.statusText;
        throw new Error(`Customer Account API HTTP ${res.status}: ${detail}`);
      }
