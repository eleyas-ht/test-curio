...
export async function addItem(
  ...
  quantity = 1,
  options: { open?: boolean; attributes?: { key: string; value: string }[] } = {},
  ...
  try {
    const data = applyResult(
      await post('/api/cart/add', { merchandiseId, quantity, ...(attributes ? { attributes } : {}) }),
    );
...
/**
 * Apply discount codes (replaces any existing). Returns the updated
 * cart; check `cart.discountCodes[].applicable` to see if the code stuck.
 */
export async function applyDiscount(codes: string[]): Promise<MutationResponse> {
  $cartBusy.set(true);
  $cartError.set(null);
  try {
    return applyResult(await post('/api/cart/discount', { codes }));
  } catch {
    $cartError.set('Could not apply that code. Please try again.');
    return { cart: null };
  } finally {
    $cartBusy.set(false);
  }
...
export function removeDiscount(): Promise<MutationResponse> {
  return applyDiscount([]);
}

/** Save an order note to the cart. */
export async function updateNote(note: string): Promise<MutationResponse> {
  $cartBusy.set(true);
  $cartError.set(null);
  try {
    return applyResult(await post('/api/cart/note', { note }));
  } catch {
    $cartError.set('Could not save the note. Please try again.');
    return { cart: null };
  } finally {
    $cartBusy.set(false);
  }
